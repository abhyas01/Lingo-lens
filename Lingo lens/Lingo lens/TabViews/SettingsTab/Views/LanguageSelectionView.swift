...
struct LanguageSelectionView: View {
    ...
    var body: some View {
        ...
        List(translationService.availableLanguages, id: \.id) { language in
            ...
            .onTapGesture {
                Logger.debug("👆 Selected language: \(language.localizedName()) (\(language.shortName()))")
        ...
        .onChange(of: translationService.availableLanguages) {
            Logger.debug(" Available languages list updated, count: \(translationService.availableLanguages.count)")
            if !translationService.availableLanguages.contains(where: { $0.shortName() == tempSelectedLanguage.shortName() }) {
                Logger.warning(" Previously selected language no longer available, updating selection")
        ...
        .onAppear {
            Logger.debug("📄 Language selection view appeared - current language: \(selectedLanguage.localizedName()) (\(selectedLanguage.shortName()))")
        ...
        .toolbar {
            ToolbarItem(placement: .cancellationAction) {
                Button("Cancel") {
                    Logger.debug("👆 Button pressed: Cancel language selection")
                    dismiss()
                }
            }
            ...
            ToolbarItem(placement: .confirmationAction) {
                Button("Done") {
                    Logger.debug("👆 Button pressed: Confirm language selection: \(tempSelectedLanguage.localizedName()) (\(tempSelectedLanguage.shortName()))")
                    isDownloading = true
        ...
        .translationTask(downloadConfig) { session in
            ...
            Logger.debug(" Preparing translation with language: \(tempSelectedLanguage.shortName())")
            do {
                
                // Try to prepare translation with selected language
                try await session.prepareTranslation()
                Logger.info(" Successfully prepared language: \(tempSelectedLanguage.shortName())")
            
                await MainActor.run {
                    isDownloading = false
                    selectedLanguage = tempSelectedLanguage
                    Logger.debug("👍 Language selection confirmed and view dismissed")
                    dismiss()
                    downloadConfig = nil
                }
            } catch {
                Logger.error(" Failed to prepare language \(tempSelectedLanguage.shortName()): \(error.localizedDescription)")
                ...
                await MainActor.run {
                    ...
                    downloadConfig = nil
                    Logger.warning(" Showing download error alert for language: \(tempSelectedLanguage.shortName())")
                }
            }
        }
    }
